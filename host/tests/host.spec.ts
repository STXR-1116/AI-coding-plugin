...
describe('TeamSkillHost', () => {
  ...
  it('installs an authorized platform artifact and reports progress without a local path', async () => {
    ...
    const fetch = vi.fn<typeof globalThis.fetch>()
      ...
      .mockImplementation(async (input) => String(input).includes('/team-skills/release-status')
    ...
    expect(await readFile(join(projectRoot, '.dsh', 'skills', RUNTIME_NAME, 'SKILL.md'), 'utf8'))
      .toContain('团队代码评审流程')
  ...
  it('uninstalls a managed copy and confirms that native discovery no longer sees it', async () => {
    ...
    const archive = artifact()
    const fetch = vi.fn<typeof globalThis.fetch>()
      .mockResolvedValueOnce(response({ operation_id: 'operation-1', status: 'authorized', skill_id: 'skill-1', runtime_name: RUNTIME_NAME, version: VERSION, artifact: { download_url: 'https://service.example.test/downloads/operation-1', expires_at: '2026-08-29T12:00:00Z', sha256: sha256(archive), size_bytes: archive.byteLength, files: [{ path: 'SKILL.md', sha256: sha256(strToU8(`---\nname: ${RUNTIME_NAME}\ndescription: 团队代码评审流程\n---\n`)) }] } }))
      .mockResolvedValueOnce(response({ accepted: true }))
      .mockResolvedValueOnce(new Response(new Uint8Array(archive).buffer))
      .mockImplementation(async () => response({ accepted: true }))
    const discovered: boolean[] = []
    const host = new TeamSkillHost({ apiBaseUrl: 'https://service.example.test/v1', accessToken: 'token-1', stateDirectory: join(root, 'state'), globalSkillRoot: join(root, 'global-skills'), fetch, refreshSkillCatalog: async (_scope, _workspacePath, _runtimeName, expectedPresent = true) => { discovered.push(expectedPresent); return true } })
    const installed = await host.install({ skillId: 'skill-1', version: VERSION, projectId: 'project-alpha', scope: 'global', environment: { dshVersion: '0.1.1', availableTools: [], availableMcpServers: [], presentEnvironmentVariableNames: [] } })
  ...
  it('quarantines and hides a managed copy omitted from the authoritative release response', async () => {
    ...
    expect(JSON.parse(String(releaseStatusCall?.[1]?.body))).toEqual({ items: [{ skill_id: 'skill-1', version: VERSION, project_id: 'project-alpha' }] })
    expect(discovered).toEqual([true, false])
  })
})
