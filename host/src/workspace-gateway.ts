 ...
 */
export function validateWorkspaceGatewayConfig(config: WorkspaceGatewayConfig): WorkspaceGatewayConfig {
...
export class WorkspaceGateway extends TypertRemoteService {
  ...
  constructor(ctx: Context, config: WorkspaceGatewayConfig) {
    ...
    this.host = new WorkspaceHost({
      ...
      session: accountSessionProvider(credentials, resolved.authMode ?? 'account', resolved.accessToken),
    })
