...
import { WorkspaceHost } from '@deepseek-ai/dsh-ai-coding-platform/workspace-host'
...
describe('CloudWorkspacesView', () => {
  ...
  it('loads the remote file tree and shows file previews in the right pane', async () => {
    ...
    expect(await screen.findByText(/# 云工作空间/u)).toBeTruthy()
  })
  ...
  it('shows the run error code and stage timeline from the run detail', async () => {
    const { remote, calls } = fakeRemote({
      ...
      run: vi.fn().mockResolvedValue(ready({
        ...
        errorCode: 'RUN_TIMEOUT',
        timeline: [
          { status: 'queued', at: '2026-09-09T00:00:00Z' },
          { status: 'failed', at: '2026-09-09T00:01:00Z', detail: '超时' },
        ],
      })),
    })
    render(
    ...
    expect(screen.getByText(/RUN_TIMEOUT/u)).toBeTruthy()
    fireEvent.click(await screen.findByRole('button', { name: /时间线/u }))
    await waitFor(() => {
...
class WorkspaceHttpFixture {
  ...
  private async createRun(
    ...
    const busy = [...this.runs.values()].find(run =>
      ...
      && ['queued', 'starting', 'running', 'waiting_approval'].includes(String(run.status)))
    if (busy !== undefined) {
