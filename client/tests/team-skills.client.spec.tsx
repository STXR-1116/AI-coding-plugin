...
const withWorkspace = <S,>(selector: (state: WorkspaceListState) => S): S => selector(workspaceState([{ workspaceId: 'ws-1' as WorkspaceId, title: 'AI开放平台', path: 'hidden', sessionIds: [], createdAt: '2026-08-29T08:00:00Z', updatedAt: '2026-08-29T08:00:00Z' }]))
const withoutWorkspace = <S,>(selector: (state: WorkspaceListState) => S): S => selector(workspaceState([]))

function remoteFor(catalog: () => Promise<unknown>, installations: () => Promise<unknown>, install: (request: unknown) => Promise<unknown>, uninstall: (request: unknown) => Promise<unknown> = async () => ({ ok: true, value: [] })): ClientRemote {
  return { teamSkills: { catalog, installations, syncReleaseStatus: installations, installSkill: install, uninstallSkill: uninstall } } as unknown as ClientRemote
}
...
const catalog = {
  ...
  catalog: {
    items: [{
      skillId: 'skill-review', displayName: '代码评审', runtimeName: 'code-review',
      summary: '按团队规范检查风险、测试和变更边界。', version: '2.4.0',
      category: '质量', tags: ['质量', '审核'], publishedAt: '2026-08-29T08:00:00Z',
    }],
  },
}
...
describe('TeamSkillsView', () => {
  it('loads the server catalog and local installations without fake fallback data', async () => {
    ...
    await waitFor(() =>{  expect(screen.getByRole('heading', { name: '代码评审' })).toBeTruthy(); })
  ...
  it('requires a scope and sends the opaque workspace id for project installation', async () => {
    const installCall = vi.fn(async () => ({
      ok: true,
      value: { status: 'succeeded', installation: {
        localInstallationId: 'local-1', skillId: 'skill-review', projectId: 'project-alpha', scope: 'project',
        workspaceId: 'ws-1', runtimeName: 'code-review', version: '2.4.0',
        artifactSha256: 'abc', state: 'normal', installedAt: '2026-08-29T08:10:00Z',
      } },
    }))
    render(<TeamSkillsView
      remote={remoteFor(async () => ({ ok: true, value: catalog }), async () => ({ ok: true, value: [] }), installCall)}
      useWorkspaces={withWorkspace}
      environment={environment}
      projectId="project-alpha"
      projects={[]}
    ...
    await waitFor(() =>{  expect(installCall).toHaveBeenCalledWith(expect.objectContaining({
      skillId: 'skill-review', version: '2.4.0', scope: 'project', workspaceId: 'ws-1', environment,
    })); })
  ...
  it('disables project scope when there is no workspace and surfaces not-ready state', async () => {
    render(<TeamSkillsView
      remote={remoteFor(async () => ({ ok: true, value: catalog }), async () => ({ ok: true, value: [] }), vi.fn())}
  ...
  it('renders service not-ready instead of a successful local fixture', async () => {
    render(<TeamSkillsView
      remote={remoteFor(async () => ({ ok: true, value: { status: 'not-ready', missing: ['apiBaseUrl'] } }), async () => ({ ok: true, value: [] }), vi.fn())}
      useWorkspaces={withoutWorkspace}
      environment={environment}
      projectId="project-alpha"
      projects={[]}
