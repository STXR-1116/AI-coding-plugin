...
export type PlatformSurfaceProps = PropsRuntime<'shell.overlay'> &
  PropsLocale<typeof NS> & { controller: PlatformDemoController; remote: ClientRemote }
...
function PlatformShell({
  ...
  useSessions,
  useWorkspaces,
}: Pick<PlatformSurfaceProps, 'controller' | 't' | 'remote' | 'useSessions' | 'useWorkspaces'>) {
  ...
  const knowledgeBinding = useRef<
    { readonly sessionId: string; readonly projectId: string; readonly knowledgeBaseIds: readonly string[] } | undefined
  >()
  ...
  useEffect(() => {
    ...
    const operation =
      projectId !== undefined && sessionId !== undefined
        ? remote.teamSkills.configureProjectMemory(sessionId, projectId)
        : sessionId === undefined
          ? Promise.resolve(undefined)
          : remote.teamSkills.clearProjectMemory(sessionId)
    void Promise.resolve(operation).catch((error: unknown) => {
      if (projectId !== undefined) setMessage(remoteFailureMessage(error))
    })
  ...
  useEffect(() => {
    ...
    void remote.teamSkills
      .memoryList({ projectId, ...(memoryKeyword === undefined ? {} : { keyword: memoryKeyword }), limit: 50 })
      .then((result) => {
  ...
  const visibleAssets = useMemo(() => {
    ...
    return [...unique.values()]
      ...
      .filter(
        item => organizationFilterId === undefined || item.organizationId === undefined || item.organizationId === organizationFilterId,
      )
  }, [access, organizationFilterId, projectAssets, projectId])
  const visibleResourceIds = (assetType: TeamSkillAsset['assetType']): readonly string[] =>
    visibleAssets
      ...
      <main className={css.main}>
        <header className={css.topbar}>
          <div className={css.breadcrumb}>
            ...
            <span>/</span>
            <strong>
              {gate === 'ready' ? NAV_ITEMS.find(item => item.id === view)?.label : gate === 'signed-out' ? '登录' : '账号验证'}
            </strong>
          </div>
          ...
          <div className={css.content}>
            ...
            {view === 'skills' && (
              <TeamSkillsView
                ...
                visibleSkillIds={visibleResourceIds('skill')}
            ...
            {view === 'collector' && (
              <CollectorView
                paused={collectorPaused}
                onToggle={() => {
                  setCollectorPaused(value => !value)
                }}
              />
            )}
  ...
  return (
    <div className={css.page}>
      <PageIntro
        eyebrow="资产总览"
        title="从权限范围内的资产开始协作"
        description="项目、Skill、知识库和记忆由服务端按账号权限返回；进入具体项目或安装项目级 Skill 时再选择项目。"
      />
      <div className={css.metricGrid}>
        ...
        <Metric
          label="知识库"
          value={String(counts.get('knowledge') ?? 0)}
          detail="当前账号可见"
    ...
    return (
      <div className={css.page}>
        <PageIntro
          eyebrow="项目"
          title="选择要查看的项目"
          description="项目是与 Skill、知识库和记忆同级的资产；详情页只读取服务端返回的项目摘要。"
        />
        <section className={css.panel}>
  ...
  return (
    <div className={css.page}>
      <PageIntro
        ...
        title="让规范在对话开始前就到位"
        description={
          projectId === undefined ? '请先选择 active 项目。' : '只检索当前项目显式开启的知识库，检索状态和引用来源由服务端返回。'
        }
        action={
          <form
          ...
          >
            ...
            <button
              type="submit"
              aria-label="检索"
              disabled={projectId === undefined || selectedIds.size === 0 || query.trim().length === 0}
            >
                ...
                {search.results.map(item => (
                  <div key={item.knowledgeId} className={css.knowledgeRow}>
                    ...
                    <button
                      type="button"
                      className={css.outlineButton}
                      onClick={() => void onPreview(item.knowledgeBaseId, item.knowledgeId)}
                    >
                      预览
                    </button>
...
function MemoryView({
  ...
  const selected = memories.find(item => item.memoryId === selectedId)
  ...
  const save = async (): Promise<void> => {
    if (selected === undefined || content.trim().length === 0 || busy) return
    setBusy(true)
    setActionMessage(undefined)
    try {
      ...
      setEditing(false)
      setActionMessage(`记忆已提交，任务 ${result.value.jobId} 处理中。`)
      onRefresh()
    } finally {
      setBusy(false)
    }
  ...
  const remove = async (): Promise<void> => {
    if (selected === undefined || busy || !window.confirm('删除后该记忆将立即不再参与召回，确认删除？')) return
    setBusy(true)
    setActionMessage(undefined)
    try {
      const result = await remote.teamSkills.memoryDelete(
        { memoryId: selected.memoryId, expectedRevision: selected.revision },
        crypto.randomUUID(),
      )
      if (!result.ok) {
        setActionMessage(result.error.message)
      ...
      setSelectedId(undefined)
      setContent('')
      setActionMessage(`记忆已删除，清理任务 ${result.value.jobId} 已提交。`)
      onRefresh()
    } finally {
      setBusy(false)
    }
  ...
  return (
    <div className={css.page}>
      <PageIntro
        ...
        description={
          projectId === undefined ? '请先选择 active 项目。' : '记忆只来自当前项目的服务端列表；自动捕获始终开启，服务不可用不会阻塞编码。'
        }
        action={
            ...
            <div className={css.memoryList}>
              {memories.map(memory => (
                <button
                  key={memory.memoryId}
                  type="button"
                  className={selectedId === memory.memoryId ? `${css.memoryRow} ${css.memoryRowActive}` : css.memoryRow}
...
function CollectorView({ paused, onToggle }: { paused: boolean; onToggle: () => void }) {
  return (
    <div className={css.page}>
      <PageIntro
        eyebrow="数据采集"
        title="看见每一次 AI Coding 的消耗"
        description="插件侧采集必要指标并脱敏上报；服务端负责校验、去重和聚合。当前页面使用本地演示批次。"
        action={
          <button
            type="button"
            className={paused ? `${css.outlineButton} ${css.resumeButton}` : css.outlineButton}
            aria-pressed={paused}
            onClick={onToggle}
          >
            {paused ? (
      ...
      />
      <div className={css.metricGrid}>
        <Metric label="今日令牌" value="6,842" detail="输入 4,920 · 输出 1,922" icon={<IconDataOutline16 size={16} />} />
        <Metric label="模型调用" value="24" detail="成功率 95.8%" icon={<IconSparkle16 size={16} />} />
        <Metric label="工具调用" value="61" detail="读取文件占 48%" icon={<IconCodeOutline16 size={16} />} />
        <Metric label="平均响应" value="18.4s" detail="较昨日下降 2.1s" icon={<IconRefreshOutline16 size={16} />} />
      </div>
      <div className={css.collectorGrid}>
        <section className={css.panel}>
          <SectionHeading title="使用趋势" action={<span className={css.periodTag}>最近 7 天</span>} />
          <div className={css.chart}>
            ...
            <div className={css.chartBars}>
              {[42, 56, 38, 64, 51, 76, 68].map((height, index) => (
                <div key={index} className={css.chartColumn}>
                  <i style={{ height: `${height}%` }} />
                  <span>{['一', '二', '三', '四', '五', '六', '今'][index]}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className={css.panel}>
          <SectionHeading
            title="实时事件"
            action={<span className={paused ? css.pausedTag : css.collectingTag}>{paused ? '已暂停' : '采集中'}</span>}
          />
          <div className={css.eventList}>
            {EVENTS.map(event => (
              <div key={`${event.time}-${event.type}`} className={css.eventRow}>
      ...
      <div className={css.collectionNote}>
        ...
        <span>默认不采集 API Key、凭证和不必要的源代码正文。采集失败不会阻塞主对话。</span>
        <strong>批次 #demo-0827-0944</strong>
      </div>
    </div>
  )
}
        ...
        <section className={`${css.panel} ${css.agentDetail}`}>
          ...
          <div className={css.agentSection}>
            ...
            <div className={css.agentToggleRow}>
              ...
              <span
                className={agent.autoContext ? `${css.toggle} ${css.toggleOn}` : css.toggle}
                aria-label={agent.autoContext ? '已开启' : '已关闭'}
              >
                <i />
              </span>
            </div>
...
function groupProjects(
  projects: readonly TeamSkillProject[],
): readonly { readonly organizationId: string; readonly organizationName: string; readonly projects: readonly TeamSkillProject[] }[] {
  const groups = new Map<
    string,
    { readonly organizationId: string; readonly organizationName: string; readonly projects: TeamSkillProject[] }
  >()
